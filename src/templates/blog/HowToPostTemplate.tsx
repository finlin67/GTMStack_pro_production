import type { AdaptedHowToPost } from '@/src/types/blog'

type Props = {
  post: AdaptedHowToPost
}
...
export default function HowToPostTemplate({ post }: Props) {
  const formattedDate = new Date(post.publishedAt).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  })
  const authorInitials = (post.authorName || 'GTMStack Expert')
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2)
      ...
      <div className="mx-auto max-w-6xl px-6 py-8 md:px-8 md:py-12">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-w-0 space-y-8">
            {post.showTakeaways && post.takeaways.length > 0 ? (
              <section className="rounded-[1.6rem] border border-blue-200/80 bg-gradient-to-br from-blue-50 to-white p-6 shadow-[0_14px_34px_rgba(59,130,246,0.08)]">
                <h2 className="text-lg font-semibold tracking-[-0.02em] text-slate-900">Key takeaways</h2>
            ...
            {post.checklistItems.length > 0 ? (
              <section className="rounded-[1.6rem] border border-emerald-200/80 bg-gradient-to-br from-emerald-50 to-white p-6 shadow-[0_14px_34px_rgba(16,185,129,0.08)]">
                <h2 className="text-lg font-semibold tracking-[-0.02em] text-slate-900">
                  {post.checklistTitle || 'Implementation checklist'}
                </h2>
                <ul className="mt-4 space-y-3">
              ...
              <section className="rounded-[1.6rem] border border-slate-200/90 bg-white p-6 shadow-[0_12px_36px_rgba(15,23,42,0.06)]">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-500">
                    {sections.length || post.takeaways.length || 0} items
                  </span>
                </div>
                <ul className="mt-5 space-y-3.5 text-sm leading-6 text-slate-600">
                  {sections.length ? (
                    ...
                    <li className="text-slate-500">No takeaways added yet.</li>
                  )}
                </ul>
              </section>
                ...
                <div className="mt-5 flex items-start gap-4">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-900">
                      {post.authorName || 'GTMStack Expert'}
                    </p>
                    <p className="mt-2 text-sm leading-6 text-slate-600">
                      Operator-led insights on B2B growth systems, GTM execution, and revenue architecture.
                    </p>
                  </div>
                </div>
              </section>

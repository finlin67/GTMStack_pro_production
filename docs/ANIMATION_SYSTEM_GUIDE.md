...
10. [Technical View: How `.tsx` Animations Are Built](#technical-view-how-tsx-animations-are-built)
...
## Technical View: How `.tsx` Animations Are Built

This section is intended as a practical handoff for developers who need to **edit, refine, or replace** existing animation components.

### 1) Generation Model (What is generated vs handwritten)

The animation system combines **hand-authored React components** with **generated metadata**:

- **Hand-authored animation code** lives in:
  - `src/components/animations/*.tsx`
- **Runtime registries (hand-authored)**:
  - `src/data/animations.ts` (dynamic imports + route/category metadata)
  - `lib/heroVisualRegistry.ts` (single-hero direct imports)
- **Generated catalog metadata**:
  - `src/data/animationCatalog.generated.ts`
  - generated by `scripts/gen-animation-catalog.ts`

Important distinction:

- `ANIMATION_REGISTRY` / `HERO_VISUAL_REGISTRY` control what appears on pages.
- `ANIMATION_CATALOG` (generated) enriches titles/descriptions/tags/thumbnails and supports gallery/admin tooling.

If metadata appears stale, regenerate catalog:
...
### 2) Typical `.tsx` Animation File Structure

Most animation files follow this shape:
...
function SmallSubComponent(...) { ... }

export default function MyAnimationTile() {
  const [state, setState] = useState(...)

  useEffect(() => {
    // setInterval / setTimeout loop for simulated live updates
...
- **State-driven simulation loops** (`setTimeout`/`setInterval`) for “live dashboard” behavior.
- **Framer Motion orchestration** (`motion.div`, `motion.path`, `AnimatePresence`) for entry/exit and continuous motion.
- **Tailwind-heavy styling** (including arbitrary color values like `bg-[#101522]`).
- **Optional helper subcomponents in same file** (`StatWidget`, `WorkflowItem`, `LayerButton`, etc.) for composition.

---

### 3) Runtime Container & Scaling Contract

Many hero renderers assume animation artboards are designed around **600×600**:

- `components/expertise/ExpertiseHeroVisual.client.tsx`
  - `ANIMATION_DESIGN_SIZE = 600`
  - wraps component in a scaled container
- `components/industries/IndustryPageContent.tsx`
  - same 600 baseline with configurable scale (`ANIMATION_SCALE = 0.85`)

Developer implication:

- Prefer animations that behave predictably in a square artboard.
- Avoid layouts that depend on full-page viewport units unless intentionally wrapped.
- If an animation looks clipped, check wrapper scale/container dimensions before rewriting internals.

---

### 4) Editing Playbook (Safest way to refine animation behavior)

When changing an animation component, edit in this order:

1. **Visual tokens first**
   - Update Tailwind classes/colors/opacity/spacing.
2. **Motion tuning second**
   - Adjust `duration`, `delay`, `repeat`, `ease`, spring `stiffness/damping`.
3. **Data simulation cadence third**
   - Tune timer intervals and random ranges.
4. **Performance pass**
   - Ensure timers are cleaned up in `useEffect` return.
   - Reduce expensive blur/shadow layers if animation stutters.

Examples:

- Slow animation globally:
  - increase durations in `transition` objects.
- Reduce jitter/noise:
  - narrow random deltas in simulated metrics updates.
- Reduce perceived intensity:
  - lower alpha/blur, reduce pulse frequency, decrease particle spawn rate.

---

### 5) Registration & ID Touchpoints After Editing/Adding

When adding a **new** animation file (not just refining existing), validate all of these:

1. Add/register in one (or both) runtime registries:
   - `src/data/animations.ts` (dynamic import variant system)
   - `lib/heroVisualRegistry.ts` (single fixed route hero)
2. Ensure `id` is stable and kebab-case.
3. Keep filename ↔ ID mapping aligned in:
   - `docs/ANIMATION_ID_REFERENCE.csv`
4. Regenerate catalog metadata if needed:
   - `npm run gen:animations`
5. Verify page resolution path:
   - expertise routes: `components/expertise/ExpertiseHeroVisual.client.tsx`
   - industry routes: `components/industries/IndustryPageContent.tsx`

---

### 6) Common Implementation Types in This Repo

From sampled files (`EocLogisticsDashboard.tsx`, `EngineeringExcellenceWorkflowTile.tsx`, `LeadGenTileAnimation.tsx`), most animations are one of:

- **Dashboard simulation tiles**
  - card grids, status pills, charts, route overlays, KPI counters.
- **Workflow/timeline narratives**
  - sequential stages with staggered reveal and progress bars.
- **Particle/funnel systems**
  - spawned entities moving through stage coordinates with conversion logic.

This means edits are usually a combination of:

- UI composition (Tailwind),
- motion sequencing (Framer Motion),
- synthetic state evolution (hooks + timers).

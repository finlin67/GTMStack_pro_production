...
function loadOverrides(): Record<string, { title?: string; description?: string; keywords?: string[]; repoUrl?: string; thumbnailSrc?: string }> {
  if (!fs.existsSync(OVERRIDES_JSON)) return {}
  try {
    return JSON.parse(fs.readFileSync(OVERRIDES_JSON, 'utf8')) as Record<string, { title?: string; description?: string; keywords?: string[]; repoUrl?: string; thumbnailSrc?: string }>
  } catch {
    return {}
  }
}
...
function run(): void {
  ...
  const itemsArray = catalogItems.map(item => {
    ...
    if (item.repoUrl) lines.push(`    repoUrl: ${JSON.stringify(item.repoUrl)},`)
    if (item.description) lines.push(`    description: ${JSON.stringify(item.description)},`)
    if (item.keywords) lines.push(`    keywords: ${JSON.stringify(item.keywords)},`)
    lines.push(`    usedOnPages: ${item.usedOnPages},`)
    return `  {\n${lines.join('\n')}\n  }`
  }).join(',\n')
